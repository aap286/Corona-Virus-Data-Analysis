...
countries_confirmed %>% filter(`Country/Region`== "Australia") %>% select(`3/22/20`) %>% sum()
```
...
```{r}
# Group by countries
counteries_recovered <- recovered %>% group_by(`Country/Region`) %>% summarise_at(vars(-Lat, -Long, -`Province/State`),sum) 

us_total <- countries_confirmed %>% filter(`Country/Region` == "US") %>% select(`3/22/20`) 
us_recover <- counteries_recovered %>% filter(`Country/Region` == "US") %>% select(`3/22/20`) 
us_recover/ us_total * 100
```
...
recovered %>% filter(`Country/Region` == "US") %>% select(`3/22/20`)
```
...
max_recovered <- counteries_recovered %>% filter(`Country/Region`== "US") %>% select(-`Country/Region`) %>% max()
...
transposed_confirmed <- transposed_confirmed %>% apply( 2, as.numeric) %>% as.data.frame()

select(transposed_confirmed,US) %>% max()
```
...
us_confirmed <- transposed_confirmed %>% select(US) %>% max()
us_confirmed
```
